...
entity top_MULT_S69xS69_10x_small is
  port (
    CLK  : in  std_logic;
    AIN  : in  std_logic;
    BIN  : in  std_logic;
...
end top_MULT_S69xS69_10x_small;

architecture rtl of top_MULT_S69xS69_10x_small is
...
begin
  ...
  U0 : entity work.MULT_S69xS69_PDm_small
    generic map (m => 5, A_LEN => N, B_LEN => N, s => 1)
    port map (CLK => CLK, A => A, B => B, O => O0);

  U1 : entity work.MULT_S69xS69_PDm_small
    generic map (m => 4, A_LEN => N, B_LEN => N, s => 1)
    port map (CLK => CLK, A => A, B => B, O => O1);

  U2 : entity work.MULT_S69xS69_PDm_small
    generic map (m => 4, A_LEN => N, B_LEN => N, s => 1)
    port map (CLK => CLK, A => A, B => B, O => O2);

  U3 : entity work.MULT_S69xS69_PDm_small
    generic map (m => 4, A_LEN => N, B_LEN => N, s => 1)
    port map (CLK => CLK, A => A, B => B, O => O3);

  U4 : entity work.MULT_S69xS69_PDm_small
    generic map (m => 4, A_LEN => N, B_LEN => N, s => 1)
    port map (CLK => CLK, A => A, B => B, O => O4);

  U5 : entity work.MULT_S69xS69_PDm_small
    generic map (m => 4, A_LEN => N, B_LEN => N, s => 1)
    port map (CLK => CLK, A => A, B => B, O => O5);

  U6 : entity work.MULT_S69xS69_PDm_small
    generic map (m => 4, A_LEN => N, B_LEN => N, s => 1)
    port map (CLK => CLK, A => A, B => B, O => O6);

  U7 : entity work.MULT_S69xS69_PDm_small
    generic map (m => 4, A_LEN => N, B_LEN => N, s => 1)
    port map (CLK => CLK, A => A, B => B, O => O7);

  U8 : entity work.MULT_S69xS69_PDm_small
    generic map (m => 4, A_LEN => N, B_LEN => N, s => 1)
    port map (CLK => CLK, A => A, B => B, O => O8);

  U9 : entity work.MULT_S69xS69_PDm_small
    generic map (m => 4, A_LEN => N, B_LEN => N, s => 1)
